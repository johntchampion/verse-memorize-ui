...
export default function Practicing() {
  ...
  const dueCount = session.data
    ? new Set(session.data.exercises.filter((e) => e.queue === 'review').map((e) => e.verseId)).size
    : 0;

  return (
    <>
      <main className="shell shell-tabbed">
        <h1 className="view-title">In practice now</h1>
        <p className="view-sub">Three at a time — never a pile. A verse leaves once it&rsquo;s kept.</p>

        <section className="stack" style={{ gap: 12, marginTop: 20 }} aria-label="Learning slots">
          ...
          <div className="due-card">
            ...
            <div className="due-row">
              ...
              <span className="due-what">
                kept {dueCount === 1 ? 'verse' : 'verses'} due for typed recall
              </span>
            </div>
            <p className="due-copy">
              They return right when you&rsquo;d start to forget. Included in today&rsquo;s session.
            </p>
          </div>
        )}
      </main>
